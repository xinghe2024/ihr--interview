...
serve({ fetch: app.fetch, port: env.PORT }, (info) => {
  console.log(`🚀 Ailin server running on http://localhost:${info.port}`);
  console.log(`   env=${env.NODE_ENV} llm=${env.LLM_PROVIDER} db=${isDbConfigured() ? 'connected' : 'NOT configured'}`);
});
